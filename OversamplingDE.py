...
class OversamplingDE:
    def __init__(self,
        ...
        self.len_parameter = len_parameter #基因个数，这里要优化ratio，所以为1
    ...
    def selection(self, u_list, np_list):
        ...
        """
        for i in range(0, self.NP):
            if self.OVERObj(u_list[i]) >= self.OVERObj(np_list[i]):
                np_list[i] = u_list[i]
            else:
                np_list[i] = np_list[i]
        return np_list
    ...
    def process(self):
        ...
        for i in range(0, self.NP):
            xx = []
            xx.append(self.OVERObj(np_list[i]))
    ...
    def OVERObj(self, OVERParam):
        """传入所有参数计算一个fpa值
        param: ratio: OVER的比例

        """

        def getFPA(model, over_X, over_y, validX, validy):
            trainmodel = model.fit(over_X, over_y)
        ...
        ratio = OVERParam[0]
        over_X, over_y = Oversampling(
            X=self.trainX, Y=self.trainy, ratio=ratio).over_sampling()

        # get the FPA with bbr model.
        fpa = getFPA(model=self.classifer, over_X=over_X, over_y=over_y,

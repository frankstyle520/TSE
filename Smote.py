...
class Smote:
    ...
    def over_sampling(self):
        ...
        if self.ratio<rareinstancesize/normalinstancesize:
            p=0

        elif self.ratio < 2 * (rareinstancesize / (self.instancesize - rareinstancesize)) :
            p = int(self.ratio * (self.instancesize -
                                    rareinstancesize) - rareinstancesize)
            #print ('p:',p)
            #print ('rareinstance:',rareinstancesize)
        ...
        for i in range(p):
            ...
            '''
            self.__populate(traininginstancesX, traininginstancesY, i, nnarray, self.r)
    ...
    def __populate(self, traininginstancesX, traininginstancesY, i, nnarray, r):
        for j in range(self.index):
            ...
            dist1 = (float)((np.sum(abs(self.syntheticX[self.count] - traininginstancesX[i])**r))**(1/r))
            dist2 = (float)((np.sum(abs(self.syntheticX[self.count] - traininginstancesX[nnarray[nn]])**r))**(1/r))
            if (dist1 + dist2 != 0):
...
def main():
    ...
    smote_X, smote_y = Smote(X=X, Y=y, ratio=1.0, k=2, r=0.5).over_sampling()
